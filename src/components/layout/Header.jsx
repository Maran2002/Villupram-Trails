...
import { Moon, Sun, Menu, X, MapPin, ChevronDown, LayoutDashboard, LogOut, User } from 'lucide-react'
...
export function Header() {
  ...
  // Whether the header is on a "hero" page where it sits over a dark image
  // (not scrolled and not a non-hero page)
  const isTransparent = !isScrolled

  // Text color classes for nav links
  const navTextBase = isTransparent
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <Link href="/" className="group flex items-center gap-2">
            ...
            <span className="font-serif text-2xl font-bold text-primary-600 dark:text-primary-400 transition-colors">
              Villupuram <span className="text-neutral-800 dark:text-white font-light italic">Hub</span>
            </span>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.path}
                  className={`relative font-medium text-sm transition-colors text-primary-500 hover:text-primary-500 ${
                    isActive
                      ? 'text-primary-500'
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {user ? (
              ...
              <>
                <Link
                  href="/auth/login"
                  className="px-4 py-2 text-sm font-semibold rounded-xl border border-primary-300 dark:border-primary-700 text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
                >
                  Sign In
                </Link>
                <Link
                  href="/auth/register"
                  className="px-4 py-2 text-sm font-semibold rounded-xl bg-primary-500 hover:bg-primary-600 text-white shadow-lg shadow-primary-500/20 transition-all"
                >
                  Join Community
                </Link>
              </>
          ...
          <div className="md:hidden flex items-center gap-3">
            <button
              onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')}
              className="p-2 rounded-full bg-neutral-100 dark:bg-dark-800 text-neutral-700 dark:text-neutral-300"
            >
              {mounted ? (resolvedTheme === 'dark' ? <Moon size={18} /> : <Sun size={18} />) : <div className="w-[18px] h-[18px]" />}
            </button>
            <button
              className="p-2 -mr-2 text-neutral-800 dark:text-neutral-200"
              ...
              {user ? (
                <div className="space-y-2">
                  <div className="px-3 py-2">
                    <p className="text-xs text-neutral-400">Signed in as <span className="font-semibold text-neutral-700 dark:text-neutral-300">{user.username}</span></p>
                  </div>
                  <Link href="/dashboard" onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-2 p-3 rounded-xl text-neutral-700 dark:text-neutral-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                    <LayoutDashboard size={16} /> My Dashboard
                  </Link>
                  <button onClick={() => { handleLogout(); setMobileMenuOpen(false) }}
                    className="w-full flex items-center gap-2 p-3 rounded-xl text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Link href="/auth/login" onClick={() => setMobileMenuOpen(false)}
                    className="text-center py-2.5 rounded-xl border border-primary-300 dark:border-primary-700 text-primary-600 dark:text-primary-400 text-sm font-semibold">
                    Sign In
                  </Link>
                  <Link href="/auth/register" onClick={() => setMobileMenuOpen(false)}
                    className="text-center py-2.5 rounded-xl bg-primary-500 text-white text-sm font-semibold">
                    Join Us
                  </Link>
                </div>
              )}
